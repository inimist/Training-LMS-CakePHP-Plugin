...
if(isset($continue_last_attempt)):
	//pr($quiz); 
	//exit;
	echo $this->Element('Quiz/admin_continue_attempt');
else:
...
if($questions){
	...
	foreach($questions as $question)	{
		if($question['QuestionType']['qtype'] == 'matching-pairs'){
			...
			$mpq_response = json_encode(unserialize(@$this->request->data['TestQuestionAttempt'][$mpq_id]['responsesummary']));
		}
		$this->Training->renderTestQuestion($question, $i++);
	}
}
?>
<?php echo $this->Form->hidden('currentpage'); ?>
<?php echo $this->Form->hidden('attempt'); ?>
<?php echo $this->Form->submit(__('Next >>'), array('id'=>'questionAttemptSubmit')); ?>
...
$('input[value="Next >>"]').on('click', function(e){
	...
	$('#TestQuizAttemptAdminTestattemptForm').submit()
		}); 
